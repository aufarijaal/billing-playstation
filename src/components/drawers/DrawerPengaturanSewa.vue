...
const formPengaturanSewaRef = ref<FormInstance>();
const formPengaturanSewa = reactive<PaketSewa>({
  nama_paket: "",
  harga_12_jam: 0,
});
const formPengaturanSewaRules = reactive<FormRules>({
...
const insertPaketSewa = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      window.api
        .insertPaketSewa(formPengaturanSewa.nama_paket, formPengaturanSewa.harga_12_jam)
...
const resetForm = () => {
  formPengaturanSewaRef.value?.resetFields();
};
...
<template>
  <el-drawer v-model="store.showDrawerPengaturanSewa" title="Pengaturan Sewa" direction="btt" custom-class="drawer-pengaturan-sewa-container" size="calc(100vh - 23px)" @open="getDataPaketSewa" @closed="paketSewaData.length = 0">
    <div class="pengaturan-sewa-container">
      <el-form label-position="top" class="form-pengaturan-sewa" ref="formPengaturanSewaRef" :rules="formPengaturanSewaRules" :model="formPengaturanSewa" hide-required-asterisk>
        <el-form-item prop="nama_paket" label="Nama Paket">
          <el-input v-model="formPengaturanSewa.nama_paket" />
        </el-form-item>
        <el-form-item label="Harga / 12 Jam" prop="harga_12_jam">
          <el-input v-model.number="formPengaturanSewa.harga_12_jam" />
        </el-form-item>
        <el-form-item>
          <el-button @click="insertPaketSewa(formPengaturanSewaRef)" type="primary">Tambah</el-button>
        </el-form-item>
        <el-form-item>
      ...
      <div class="secondary-text" style="text-align: center; margin-bottom: 20px">
        Input: Nama Paket <span style="color: var(--el-color-warning)">{{ formPengaturanSewa.nama_paket }}</span
        >, Harga <span style="color: var(--el-color-success)">{{ toRupiah(formPengaturanSewa.harga_12_jam) }}</span> (1 hari <span style="color: var(--el-color-primary)">{{ toRupiah(formPengaturanSewa.harga_12_jam * 2) }}</span
        >)
      </div>
